...
BACKUP_DIR="$REPO/.wb-backups-cycle1-entities-repair"
...
if "from .entities_registry import (" not in text:
    anchor = "from .indexer import Indexer\n"
...
if '"register-entity"' not in text:
    anchor = "    return parser\n"
    if anchor not in text:
        raise SystemExit("Parser return anchor not found in cli.py")
    text = text.replace(anchor, parser_block + "    return parser\n", 1)

old_dispatch = '    raise ValueError(f"Unknown command: {args.command}")'
new_dispatch = """    if args.command == "register-entity":
...
echo "==== VALIDATION: grep CLI ===="
grep -n "register-entity" "$CLI_FILE"
grep -n "list-entities" "$CLI_FILE"
grep -n "show-entity-state" "$CLI_FILE"

echo
echo "==== VALIDATION: help output ===="
"$PYBIN" "$REPO/archivist.py" -h | sed -n '1,260p'

echo
echo "Repair patch applied successfully."
echo "CLI backup: $BACKUP_DIR/cli.py.$STAMP.bak"
echo "Entity module: $ENTITY_MODULE"
