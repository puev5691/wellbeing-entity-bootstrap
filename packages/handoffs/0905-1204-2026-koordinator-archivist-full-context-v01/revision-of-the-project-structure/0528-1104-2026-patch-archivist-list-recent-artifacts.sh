...
if "def list_recent_artifacts(" not in text:
    addition = """
...
if "list-recent-artifacts" not in text:
...
"""
    ...
    parser_anchor = """    list_confirmed_steps_cmd.add_argument("--callsign", help="Entity callsign")
    list_confirmed_steps_cmd.add_argument("--entity-id", dest="entity_id", type=int, help="Entity id")

    return parser
"""
    parser_addition = """    list_confirmed_steps_cmd.add_argument("--callsign", help="Entity callsign")
    list_confirmed_steps_cmd.add_argument("--entity-id", dest="entity_id", type=int, help="Entity id")
...
"""
    if parser_anchor not in text:
        raise SystemExit("Parser anchor for list-recent-artifacts not found in cli.py")
    text = text.replace(parser_anchor, parser_addition, 1)

    dispatch_anchor = """    if args.command == "list-confirmed-steps":
        return cmd_list_confirmed_steps(args)

    parser.error(f"Unknown command: {args.command}")
    return 2
"""
    dispatch_addition = """    if args.command == "list-confirmed-steps":
        return cmd_list_confirmed_steps(args)
...
"""
    if dispatch_anchor not in text:
        raise SystemExit("Dispatch anchor for list-recent-artifacts not found in cli.py")
...
"""
    if handler_anchor not in text:
        raise SystemExit("Handler anchor for list-recent-artifacts not found in cli.py")
