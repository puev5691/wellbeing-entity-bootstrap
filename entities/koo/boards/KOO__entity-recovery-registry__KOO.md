# Реестр апгрейда Сущностей под recovery v1.2 — v0.3
...
| SHT | ШТАБИСТ | входит в проект, но не в текущую очередь миграции | не требуется на этом этапе | `deferred_role_design` | ОПЕРАТОР планирует внутреннюю административно-организационную функцию по типу КАНЦЕЛЯРА, но профиль пока не формализован | отложить проектирование и recovery до отдельной постановки; сейчас поднимать однозначно нужные рабочие Сущности |
...
## Исправление границ и очереди

Версия v0.1 ошибочно включала ШКОЛУ в очередь текущего проекта и трактовала ВОЛОНТЁРА как потенциальную Сущность для recovery. Версия v0.2 оставляла ШТАБИСТА как blocker из-за неуточнённой роли.
...
- ШТАБИСТ задуман для внутреннего административно-организационного контура по типу КАНЦЕЛЯРА, но пока не должен отвлекать текущую кампанию; его проектирование и recovery отложены до отдельной постановки.

Эти пункты считаются текущей границей кампании и не должны снова попадать в очередь без отдельного решения ОПЕРАТОРА.
...
supersedes: KOO__entity-recovery-registry v0.2
